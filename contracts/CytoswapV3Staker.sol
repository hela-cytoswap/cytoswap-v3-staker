...
import './interfaces/ICytoswapV3Staker.sol';
...
contract CytoswapV3Staker is ICytoswapV3Staker, Multicall {
    ...
    /// @param _factory the Cytoswap V3 factory
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward) external override {
        require(reward > 0, 'CytoswapV3Staker::createIncentive: reward must be positive');
        require(
            block.timestamp <= key.startTime,
            'CytoswapV3Staker::createIncentive: start time must be now or in the future'
        );
        require(
            key.startTime - block.timestamp <= maxIncentiveStartLeadTime,
            'CytoswapV3Staker::createIncentive: start time too far into future'
        );
        require(key.startTime < key.endTime, 'CytoswapV3Staker::createIncentive: start time must be before end time');
        require(
            key.endTime - key.startTime <= maxIncentiveDuration,
            'CytoswapV3Staker::createIncentive: incentive duration is too long'
        );
    ...
    function endIncentive(IncentiveKey memory key) external override returns (uint256 refund) {
        require(block.timestamp >= key.endTime, 'CytoswapV3Staker::endIncentive: cannot end incentive before end time');
        ...
        require(refund > 0, 'CytoswapV3Staker::endIncentive: no refund available');
        require(
            incentive.numberOfStakes == 0,
            'CytoswapV3Staker::endIncentive: cannot end incentive while deposits are staked'
        );
    ...
    /// @notice Upon receiving a Cytoswap V3 ERC721, creates the token deposit setting owner to `from`. Also stakes token
    ...
    function onERC721Received(
        ...
        require(
            ...
            'CytoswapV3Staker::onERC721Received: not a univ3 nft'
        );
    ...
    function transferDeposit(uint256 tokenId, address to) external override {
        require(to != address(0), 'CytoswapV3Staker::transferDeposit: invalid transfer recipient');
        address owner = deposits[tokenId].owner;
        require(owner == msg.sender, 'CytoswapV3Staker::transferDeposit: can only be called by deposit owner');
    ...
    function withdrawToken(
        ...
        require(to != address(this), 'CytoswapV3Staker::withdrawToken: cannot withdraw to staker');
        Deposit memory deposit = deposits[tokenId];
        require(deposit.numberOfStakes == 0, 'CytoswapV3Staker::withdrawToken: cannot withdraw token while staked');
        require(deposit.owner == msg.sender, 'CytoswapV3Staker::withdrawToken: only owner can withdraw token');
    ...
    function stakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        require(deposits[tokenId].owner == msg.sender, 'CytoswapV3Staker::stakeToken: only owner can stake token');
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        if (block.timestamp < key.endTime) {
            require(
                ...
                'CytoswapV3Staker::unstakeToken: only owner can withdraw token before incentive end time'
            );
        }
        ...
        require(liquidity != 0, 'CytoswapV3Staker::unstakeToken: stake does not exist');
        ...
        returns (uint256 reward, uint160 secondsInsideX128)
    {
        ...
        (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity) = stakes(tokenId, incentiveId);
        require(liquidity > 0, 'CytoswapV3Staker::getRewardInfo: stake does not exist');
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        require(block.timestamp >= key.startTime, 'CytoswapV3Staker::stakeToken: incentive not started');
        require(block.timestamp < key.endTime, 'CytoswapV3Staker::stakeToken: incentive ended');

        bytes32 incentiveId = IncentiveId.compute(key);

        require(
            incentives[incentiveId].totalRewardUnclaimed > 0,
            'CytoswapV3Staker::stakeToken: non-existent incentive'
        );
        require(
        ...
        require(liquidity > 0, 'CytoswapV3Staker::stakeToken: cannot stake token with 0 liquidity');
